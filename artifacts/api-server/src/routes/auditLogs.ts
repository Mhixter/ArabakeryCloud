...
router.get("/audit-logs", authenticate, requireRole("managing_director"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const { branchId, userId, action, startDate, endDate, limit, offset } = req.query as { branchId?: string; userId?: string; action?: string; startDate?: string; endDate?: string; limit?: string; offset?: string };
  ...
  const [logs, countResult] = await Promise.all([
    db.select().from(auditLogsTable).where(whereClause).orderBy(desc(auditLogsTable.createdAt)).limit(pageLimit).offset(pageOffset),
    db.select({ count: desc(auditLogsTable.id) }).from(auditLogsTable).where(whereClause),
  ]);
  res.json({
    logs: logs.map(log => ({ id: log.id, userId: log.userId, userName: log.userName, action: log.action, entityType: log.entityType, entityId: log.entityId, details: log.details, ipAddress: log.ipAddress, branchId: log.branchId, createdAt: log.createdAt.toISOString() })),
