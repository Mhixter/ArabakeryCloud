...
router.get("/sales", authenticate, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  res.json(sales.map(({ sale, cashierName, branchName }) =>
    formatSale(sale, cashierName ?? "Unknown", branchName ?? "Unknown")
  ));
});
...
router.post("/sales", authenticate, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const costAmount = 0; // TODO: link to actual ingredient cost
  ...
  await logAudit({
    ...
    entityType: "sale",
    entityId: sale.id,
    details: `${breadType} x${qty} @ ${price} = ${totalAmount} (${paymentMethod})`,
...
router.get("/sales/daily-summary", authenticate, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const conditions = [
    ...
    gte(salesTable.saleDate, startOfDay),
    lte(salesTable.saleDate, endOfDay),
  ];
  if (branchId && !isNaN(parseInt(branchId))) {
    conditions.push(eq(salesTable.branchId, parseInt(branchId)));
  }
  ...
  const totalCost = sales.reduce((sum, s) => sum + parseFloat(s.costAmount as unknown as string), 0);
  ...
  for (const s of sales) {
    ...
    breadTypeMap.set(s.breadType, {
      quantity: prev.quantity + s.quantity,
      revenue: prev.revenue + parseFloat(s.totalAmount as unknown as string),
    });
  }
  ...
  res.json({
    ...
    cashSales,
    transferSales,
    breadTypes: Array.from(breadTypeMap.entries()).map(([breadType, data]) => ({
      breadType,
...
router.get("/sales/:id", authenticate, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const [result] = await db
    ...
    .leftJoin(usersTable, eq(salesTable.cashierId, usersTable.id))
    .leftJoin(branchesTable, eq(salesTable.branchId, branchesTable.id))
    .where(and(eq(salesTable.id, id), isNull(salesTable.deletedAt)));
