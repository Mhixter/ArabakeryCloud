...
export default function QuickSaleSettlementPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/quick-sale-settlements?branchId=${activeBranch.id}&businessDate=${businessDate}`, {
      ...
      toast({ title: "Could not load daily Quick Sales", description: error instanceof Error ? error.message : "Please try again", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }, [activeBranch?.id, businessDate, toast]);

  useEffect(() => { load(); }, [load]);

  async function acceptDay() {
    ...
    try {
       const res = await fetch(`${API_BASE}/api/quick-sale-settlements/accept`, {
        ...
        credentials: "include",
         body: JSON.stringify({ branchId: activeBranch?.id, businessDate, paymentMethod, notes: notes.trim() || undefined }),
      });
      const result = await res.json();
       if (!res.ok) throw new Error(result.error ?? "Could not accept daily settlement");
       toast({ title: "Daily Quick Sale accepted", description: `${currency(result.settlement.amount)} accepted from manager.` });
      await load();
    } catch (error) {
       toast({ title: "Daily settlement failed", description: error instanceof Error ? error.message : "Please try again", variant: "destructive" });
    } finally {
      setAccepting(false);
    }
  }
  ...
  return (
    <div className="space-y-6" data-testid="page-quick-sale-settlement">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Review manager-recorded Quick Sales and accept one business day at a time.</p>
      </div>
        ...
        <>
          <Card className="border-emerald-200 bg-emerald-50/50">
            <CardContent className="p-4 flex flex-wrap items-center justify-between gap-3">
              <div><p className="text-xs uppercase tracking-wide text-emerald-800 font-semibold">Selected business day</p><p className="font-bold text-emerald-950">{data?.businessDate ?? businessDate}</p></div>
              <Input type="date" value={businessDate} max={businessDateFor()} onChange={e => setBusinessDate(e.target.value)} className="w-[155px]" />
            </CardContent>
          </Card>
          <div className="grid gap-4 md:grid-cols-3">
            <Card><CardContent className="p-5"><p className="text-xs uppercase tracking-wide text-muted-foreground">Daily Quick Sales</p><p className="text-2xl font-bold mt-1">{currency(data?.totalAmount ?? 0)}</p></CardContent></Card>
            <Card><CardContent className="p-5"><p className="text-xs uppercase tracking-wide text-muted-foreground">Recorded entries</p><p className="text-2xl font-bold mt-1">{data?.day.count ?? 0}</p></CardContent></Card>
            <Card><CardContent className="p-5"><p className="text-xs uppercase tracking-wide text-muted-foreground">Daily status</p><p className="text-2xl font-bold mt-1">{data?.accepted ? "Accepted" : "Unaccepted"}</p></CardContent></Card>
          </div>
          <Card>
            ...
            <CardContent className="space-y-3">
              {loading ? <p className="text-sm text-muted-foreground">Loading…</p> : data?.day ? [data.day].map(day => (
               ...
               {data?.accepted?.stockClearedAt ? <div className="flex items-center gap-2 text-emerald-700 font-medium"><CheckCircle2 size={18} />Accepted {currency(data.accepted.amount)} by {data.accepted.paymentMethod}; cleared {data.accepted.stockClearedProducts} product stock balances.</div> : (
                <div className="flex flex-wrap items-end gap-3">
                  ...
                  <div><Label htmlFor="weekly-notes">Notes / reference</Label><Input id="weekly-notes" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Optional" /></div>
                    <Button onClick={acceptDay} disabled={accepting || !data || data.totalAmount <= 0}><HandCoins size={15} className="mr-2" />{accepting ? "Processing…" : data?.accepted ? "Clear remaining stock" : `Accept ${currency(data?.totalAmount ?? 0)}`}</Button>
                </div>
              )}
            </CardContent>
          </Card>
