...
All operational daily totals use calendar dates in `Africa/Lagos`, not the server or browser's local timezone. Date-only API filters are interpreted as that timezone's full day, and UI grouping uses the same timezone.
